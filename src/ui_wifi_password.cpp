...
static void drawHeader()
{
    ...
    String shownSSID = wifiSSID.length() ? wifiSSID : "<none>";
    if (shownSSID.length() > 22)
    {
        shownSSID = shownSSID.substring(0, 22);
    }

    String shownPass = wifiPASS.length() ? wifiPASS : "";
    if (shownPass.length() > 26)
    {
        shownPass = shownPass.substring(shownPass.length() - 26);
    }
    ...
    tft.drawRect(10, 66, 220, 24, TFT_GREEN);
    tft.setCursor(14, 74);
    tft.print(shownPass);
...
UiWifiPasswordAction uiWifiPasswordHandleTouch(uint16_t x, uint16_t y)
{
    ...
    {
        waitTouchRelease();
        return UI_WIFI_PASSWORD_ACTION_BACK;
    }
    ...
    if (action == UI_KEYBOARD_ACTION_CHANGED)
    {
        drawHeader();
        uiKeyboardDraw();
        return UI_WIFI_PASSWORD_ACTION_NONE;
    }

    if (action == UI_KEYBOARD_ACTION_OK)
    {
        return UI_WIFI_PASSWORD_ACTION_OK;
    }
